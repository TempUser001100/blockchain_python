...
class TransE:
    def __init__(self, kg: KnowledgeGraph,
                 ...
                 batch_size, learning_rate, n_generator, n_rank_calculator,id):
        self.kg = kg
        self.id = id
        ...
        with tf.variable_scope('embedding'):
            ...
            if len(os.listdir('../data/train_data4/')):
                en_rel = scio.loadmat('../data/train_data4/'+str(len(os.listdir('../data/train_data4/')))+'.mat')
                # entity = en_rel['entity_embedding']
                # relation = en_rel['relation_embedding']
                entity,relation = self.calcul_var(en_rel)
                self.entity_embedding = tf.Variable(entity,name='entity')
                self.relation_embedding = tf.Variable(relation,name='relation')
                # print(self.entity_embedding,self.relation_embedding)
            else:
                self.entity_embedding = tf.get_variable(name='entity',
                                                    shape=[kg.n_entity, self.embedding_dim],
                                                    initializer=tf.random_uniform_initializer(minval=-bound,
...
#    def calcul_var(self,en_rel):
#        num = len(os.listdir('../data/train_data2/'))
#        print(en_rel)
#        entity_embedding_mean = (en_rel['entity_embedding_1']+en_rel['entity_embedding_2']+en_rel['entity_embedding_3']+en_rel['entity_embedding_4'])/4
#        relation_embedding_mean = (en_rel['relation_embedding_1']+en_rel['relation_embedding_2']+en_rel['relation_embedding_3']+en_rel['relation_embedding_4'])/4
#        print(entity_embedding_mean.shape,relation_embedding_mean.shape)
#        return entity_embedding_mean,relation_embedding_mean
    def calcul_var(self,en_rel):
        en_1 = np.concatenate(self.prune(en_rel['entity_embedding_1']),axis=0)
        en_2 = np.concatenate(self.prune(en_rel['entity_embedding_2']),axis=0)
        en_3 = np.concatenate(self.prune(en_rel['entity_embedding_3']),axis=0)
        en_4 = np.concatenate(self.prune(en_rel['entity_embedding_4']),axis=0)
        rel_1 = np.concatenate(self.prune(en_rel['relation_embedding_1']),axis=0)
        rel_2 = np.concatenate(self.prune(en_rel['relation_embedding_2']),axis=0)
        rel_3 = np.concatenate(self.prune(en_rel['relation_embedding_3']),axis=0)
        rel_4 = np.concatenate(self.prune(en_rel['relation_embedding_4']),axis=0)
        # en_1 = np.concatenate(en_rel['entity_embedding_1'],axis=0)
        # en_2 = np.concatenate(en_rel['entity_embedding_2'],axis=0)
        # en_3 = np.concatenate(en_rel['entity_embedding_3'],axis=0)
        # en_4 = np.concatenate(en_rel['entity_embedding_4'],axis=0)
        # rel_1 = np.concatenate(en_rel['relation_embedding_1'],axis=0)
        # rel_2 = np.concatenate(en_rel['relation_embedding_2'],axis=0)
        # rel_3 = np.concatenate(en_rel['relation_embedding_3'],axis=0)
        # rel_4 = np.concatenate(en_rel['relation_embedding_4'],axis=0)
        ...
        _entity = PCA(n_components=1).fit_transform(_entity)
        _relation = PCA(n_components=1).fit_transform(_relation)
        # print(_entity.shape)
        _entity = _entity.reshape(-1,100)
        _relation = _relation.reshape(-1,100)
        return _entity,_relation

    def prune(self,W):
        alpha = 0.3
        # alpha = self.rate
        threshold = alpha*np.std(W.flatten())
        mask = (np.abs(W))>(np.abs(threshold))
        mask = np.bool_(mask)
        _W =W*mask
        return _W
    ...
    def launch_evaluation(self, session):
        ...
        f1 = open(str(self.id)+'hit.txt','a+')
        f2 = open(str(self.id)+'meanrank.txt','a+')
        f1.write(str((head_hits10_filter + tail_hits10_filter) / 2)+'\n')
        f2.write(str((head_meanrank_filter + tail_meanrank_filter) / 2)+'\n')
        f1.close()
        f2.close()
    ...
    def check_norm(self, session):
        ...
        # loss = self.loss.eval(session=session)
        _loss = self.epoch_losses
        ...
        return _loss,entity_embedding,relation_embedding
